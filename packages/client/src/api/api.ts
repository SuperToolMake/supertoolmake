...
export const API = createAPIClient({
  ...
  onError: (error) => {
    const { status, method, url, message, handled, suppressErrors } = error || {}
    const ignoreErrorUrls = ["bbtel", "/api/global/self", "/api/tables/ta_users"]
    ...
    if (message && !suppressErrors) {
      ...
      let ignore = false
      for (const ignoreUrl of ignoreErrorUrls) {
