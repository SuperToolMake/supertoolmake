...
const getValidOperatorsForType = (
  ...
  coreOperators = [
    ...coreOperators,
    ...(fieldType === FieldType.DATETIME ? [{ value: RangeOperator.RANGE, label: "Between" }] : []),
  ]

  // Map the operators to set an label overrides
  return coreOperators
    .filter((op) => {
      if (isDateTime && (op.value === FilterType.ONE_OF || op.value === ArrayOperator.NOT_ONE_OF))
        return false
      return true
    })
    .map((op) => ({
...
async function fetchSchema(datasource: any) {
  if (datasource) {
    const fetchedSchema: TableSchema | null = await fetchDatasourceSchema(datasource, {
...
const getOperators = (config: FilterConfig | undefined, schema: TableSchema | null) => {
...
const initRelationShips = (filters: Record<string, SearchFilter>) => {
  const filtered = Object.entries(filters || {}).filter(([_, filter]: [string, SearchFilter]) => {
...
const getRelIds = (filters: Record<string, SearchFilter>) => {
  const filtered = Object.entries(filters || {}).reduce((acc: string[], [_, filter]) => {
    if (
