...
export const name = (
  ...
  validation.addValidator(
    ...
    string()
      ...
      .matches(APP_NAME_REGEX, "Workspace name must be letters, numbers and spaces only")
      .test(
        ...
        (value) => {
          ...
          return !workspaces
            ...
            .some((workspaceName) => workspaceName.toLowerCase() === value.toLowerCase())
        }
      )
  )
}
...
export const file = (
  ...
  const templateToUse = template && Object.keys(template).length === 0 ? null : template
  validation.addValidator(
    "file",
    templateToUse?.fromFile ? mixed().required("Please choose a file to import") : null
  )
}
