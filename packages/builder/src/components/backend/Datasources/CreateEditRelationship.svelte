...
function manyToManyRelationshipExistsFn() {
  ...
  let fromThroughLinks = Object.values(datasource.entities[getTable(fromId).name].schema).filter(
    (value) => value.through
  )
  let toThroughLinks = Object.values(datasource.entities[getTable(toId).name].schema).filter(
    (value) => value.through
  )

  const matchAgainstUserInput = (link) =>
    (link.throughTo === throughToKey && link.throughFrom === throughFromKey) ||
...
function validate() {
  ...
  errors = {
    ...
    fromTable:
      errorChecker.tableSet(fromTable) || errorChecker.differentTables(fromId, toId, throughId),
    toTable:
      errorChecker.tableSet(toTable) || errorChecker.differentTables(toId, fromId, throughId),
    throughTable:
    ...
    throughFromKey:
      ...
      errorChecker.manyTypeMismatch(fromTable, throughTable, fromTable.primary[0], throughToKey) ||
      errorChecker.differentColumns(throughFromKey, throughToKey),
    throughToKey:
      errorChecker.manyForeignKeySet(throughToKey) ||
      errorChecker.manyTypeMismatch(toTable, throughTable, toTable.primary[0], throughFromKey),
    fromForeign:
    ...
    fromColumn: errorChecker.columnBeingUsed(toTable, fromColumn, originalFromColumnName),
    toColumn: errorChecker.columnBeingUsed(fromTable, toColumn, originalToColumnName),
  }
...
function removeExistingRelationship() {
  ...
  if (originalToTable && originalToColumnName) {
    delete datasource.entities[originalToTable.name].schema[originalFromColumnName]
  }
}
...
async function saveRelationship() {
  ...
  datasource.entities[getTable(fromId).name].schema[fromRelationship.name] = fromRelationship
  // save other side of relationship in the other schema
  datasource.entities[getTable(toId).name].schema[toRelationship.name] = toRelationship
...
onMount(() => {
  ...
  relationshipType = fromRelationship.relationshipType || RelationshipType.MANY_TO_ONE
