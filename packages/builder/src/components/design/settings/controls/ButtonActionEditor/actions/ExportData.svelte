...
$: components = findAllMatchingComponents($selectedScreen?.props, (component) => {
  ...
  return type.endsWith("/table") || type.endsWith("/tableblock") || type.endsWith("/gridblock")
})
$: componentOptions = components.map((table) => ({
  label: table._instanceName,
  value: table._component.endsWith("/tableblock") ? `${table._id}-table` : table._id,
}))
$: selectedTableId = parameters.tableComponentId?.includes("-")
  ? parameters.tableComponentId.split("-")[0]
  : parameters.tableComponentId
$: selectedTable = components.find((component) => component._id === selectedTableId)
