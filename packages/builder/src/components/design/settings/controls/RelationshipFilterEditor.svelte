...
import { findClosestMatchingComponent, findComponent } from "@/helpers/components"
import { componentStore, selectedScreen, tables } from "@/stores/builder"
...
const resolveDatasource = (selectedScreen, componentInstance, form) => {
  if (!form && componentInstance._id != $componentStore.selectedComponentId) {
    ...
    return def?.block === true ? getDatasourceForProvider(selectedScreen, block) : {}
  } else {
