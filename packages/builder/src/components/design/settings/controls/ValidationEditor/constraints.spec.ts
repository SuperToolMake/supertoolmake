...
describe("defaultErrorForConstraint", () => {
  ...
  it.each([
    ...
    expect(defaultErrorForConstraint(constraint, value, "datetime")).toBe(message)
  })
  ...
  ])("does not include JavaScript binding values for %s", (constraint, message) => {
    expect(defaultErrorForConstraint(constraint, jsBinding)).toBe(message)
  })

  it("returns a generic message for unknown constraints", () => {
    expect(defaultErrorForConstraint("not-a-real-constraint", null)).toBe("Invalid value")
