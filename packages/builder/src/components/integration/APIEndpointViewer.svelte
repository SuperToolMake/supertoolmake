<script lang="ts">
  ...
  let authConfigs: { label: string; value: string }[] = []
  ...
  $: {
    const key = editableQuery?._id || `new::${selectedDatasourceId || ""}`
    if (key !== defaultAuthKey) {
      defaultAuthKey = key
      defaultAuthApplied = false
    }
  }
  $: if (!defaultAuthApplied && editableQuery && datasource && isNewQuery) {
  ...
  $: template =
    hasRestTemplate(datasource) && $restTemplates
      ? restTemplates.getByName(getRestTemplateIdentifier(datasource))
      : undefined
  ...
  <div class="heading">
    ...
    <div class="actions">
      <div class="grouped">
        ...
        {/if}
        {#if editableQuery && (isCustomMode || selectedEndpointOption)}
          <AuthPicker
            bind:authConfigId={editableQuery.fields.authConfigId}
            bind:authConfigType={editableQuery.fields.authConfigType}
            {authConfigs}
