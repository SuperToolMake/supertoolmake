...
const applyEndpointDefaults = (
  ...
  const fullPath = endpoint && baseUrl ? constructFullPath(baseUrl, endpoint?.path || "") : ""
  ...
  const parameters = Object.entries(allBindings).map(([name, defaultValue]) => ({
    name,
    default: defaultValue,
  }))
  const bodyType =
    endpoint?.bodyType || (endpoint?.originalRequestBody ? BodyType.JSON : BodyType.NONE)
  let requestBody = endpoint?.originalRequestBody
  const isKeyValueBodyType = bodyType === BodyType.FORM_DATA || bodyType === BodyType.ENCODED
...
const getEndpointOptions = (endpoints: ImportEndpoint[]): EndpointWithIcon[] => {
 ...
 * @param endpoints
 */
const syncEndpointFromQuery = (query: Query, endpoints: ImportEndpoint[] | undefined) => {
  ...
  if (query._id && query.restTemplateMetadata) {
    ...
    const endpoint = {
      id: metadata.operationId || `${method.toLowerCase()}::${metadata.originalPath}`,
      name: endpointName,
      method,
 ...
 * @param endpoints
 */
function parseLegacyQuery(query: Query, endpoints: ImportEndpoint[] | undefined) {
  if (endpoints && query.fields?.path && !query.restTemplateMetadata) {
    try {
      ...
      const found = endpoints.find((e) => {
        return e.method === QUERY_VERB_MAP[query.queryVerb] && e.path === sanitized
      })
...
async function saveQuery(redirectIfNew = true) {
  ...
  try {
    ...
    const updatedVariables = rebuildVariables(_id!, dynamicVariables, existingVariables)
...
// UPDATE HANDLERS
const onUpdateParams = (e: CustomEvent<Array<{ name: string; value: string }>>) => {
...
const onUpdateHeaders = (e: CustomEvent<Array<{ name: string; value: string }>>) => {
  if (query) {
...
const moveToExpanded = (node: HTMLElement) => {
  ...
  const unsubscribe = sidebarExpanded.subscribe((expanded) => {
    ...
    if (expanded) {
      ...
      setTimeout(() => {
        const expandedTarget = document.querySelector(".side-bar.expanded .side-bar-content")
      ...
      const collapsedTarget = document.querySelector(".side-bar.main .side-bar-content")
...
const sidebarTransition = (_node: HTMLElement, params: { direction: "in" | "out" }) => {
  if (!sidebarElement) {
    return { duration: 260 }
  }
