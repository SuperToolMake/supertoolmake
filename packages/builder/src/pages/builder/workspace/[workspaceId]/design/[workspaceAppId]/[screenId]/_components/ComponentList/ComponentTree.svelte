...
import { findComponentPath, getComponentText, getComponentName } from "@/helpers/components"
...
$: filteredComponents = components?.filter(hasComponentId).filter((component) => {
  ...
  const isVisibleInComponentTree = !isSearching || visibleSearchIds.has(component._id)
...
const openContextMenu = (e: MouseEvent, component: ComponentWithId, opened: boolean) => {
  e.preventDefault()
  e.stopPropagation()
