...
import { Layout, Search, Icon, Body, notifications } from "@supertoolmake/bbui"
...
const getAllowedComponents = (allComponents, screen, component) => {
  ...
  if (definition?.legalDirectChildren?.length) {
    allowedComponents = definition.legalDirectChildren.map((x) => {
      return `@supertoolmake/standard-components/${x}`
    })
  } else {
  ...
  path.forEach((ancestor) => {
    ...
    if (
      [
        "@supertoolmake/standard-components/sidepanel",
        "@supertoolmake/standard-components/modal",
...
const enrichStructure = (structure, definitions, customComponents) => {
  ...
  structure.forEach((item) => {
    if (typeof item === "string") {
      const def = definitions[`@supertoolmake/standard-components/${item}`]
      if (def) {
        enrichedStructure.push({
          ...def,
          isCategory: false,
