...
import { builderStore, componentStore, previewStore, selectedScreen } from "@/stores/builder"
...
const eventUpdatesState = (handlers: EventHandler[] | undefined, stateKey: string) => {
  return handlers?.some((handler) => {
    return handler["##eventHandlerType"] === "Update State" && handler.parameters?.key === stateKey
  })
}
  ...
  if (setting.type === "event") {
    ...
    if (Array.isArray(components)) {
      for (let comp of components) {
        ...
        const componentSettings = componentStore.getComponentSettings(comp._component)
        for (let compSetting of componentSettings) {
          if (compSetting.type === "event" && eventUpdatesState(comp[compSetting.key], stateKey)) {
            return true
          }
        }
      }
...
const getSettingsUsingState = (component: Component, stateKey: string): string[] => {
...
const hasStateBinding = (value: string, stateKey: string): boolean => {
  const bindings = findHBSBlocks(value).map((binding) => {
    ...
    return isJSBinding(sanitizedBinding) ? decodeJSBinding(sanitizedBinding) : sanitizedBinding
  })
...
const handleStateInspectorChange = (e: CustomEvent) => {
  ...
  const stateUpdate = {
    [selectedKey]: processStringSync(e.detail, $previewStore.selectedComponentContext),
  }
