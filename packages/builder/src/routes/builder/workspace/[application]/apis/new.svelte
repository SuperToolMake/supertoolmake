...
import { formatEndpointLabel, getRestTemplateImportInfoRequest } from "@/helpers/restTemplates"
...
const buildDatasourceName = (template: RestTemplate, spec: RestTemplate["specs"][number]) => {
...
const importTemplateSelection = async (
  ...
  try {
    ...
    templateEndpoints = info.endpoints?.slice().sort((a, b) => compareEndpointOrder(a, b))
...
const importTemplate = async () => {
  ...
  try {
    ...
    const datasourcePayload: Datasource = {
      ...
      plus:
        restIntegration.plus && (restIntegration.name as SourceName) !== SourceName.REST
...
const selectTemplateGroup = (group: RestTemplateGroup<RestTemplateGroupName>) => {
