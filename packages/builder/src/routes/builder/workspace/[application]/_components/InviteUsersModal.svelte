...
const inviteUsers = async (userData: UserData) => {
  const result = await assignExistingUsersToWorkspace(userData, currentWorkspaceId)
...
const createUsers = async (userData: UserData) => {
  ...
  if (assignToWorkspace && currentWorkspaceId && createUsersResponse.successful?.length) {
