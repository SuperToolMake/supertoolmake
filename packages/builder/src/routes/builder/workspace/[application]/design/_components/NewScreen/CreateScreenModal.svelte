<script lang="ts">
import { Modal, ModalCancelFrom, notifications } from "@supertoolmake/bbui"
import type { SaveScreenRequest, Screen, Table } from "@supertoolmake/types"
...
const createScreen = async (screenTemplate: SaveScreenRequest): Promise<Screen> => {
  try {
...
const handleTableOrViewToggle = ({ detail: tableOrView }: { detail: SourceOption }) => {
  const alreadySelected = selectedTables.some((selected) => selected.id === tableOrView.id)
