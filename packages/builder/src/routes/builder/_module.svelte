...
import { admin, appsStore, auth, enrichedApps, organisation } from "@/stores/portal"
...
derivedMemo(
  ...
  ([$admin, $auth, $enrichedApps, $isActive, $appsStore, $loaded]) => {
    ...
    if ($auth.user) {
      ...
      if (
        ($appsStore.apps.length === 0 || (isBuilder && !hasEditableWorkspaces)) &&
        !$isActive("./apps") &&
        !$isActive("./onboarding") &&
      ...
      const isOnWorkspaceRoute =
        $isActive("./workspace") || $isActive("./workspace/updating")
