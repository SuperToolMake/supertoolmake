...
const sanitizeAppRole = (appRole: string) => {
  ...
  if (rolesLoaded && !endUserRoleOptions.some((option) => option.value === appRole)) {
...
const canWorkspaceRoleOverrideGlobalRole = (globalRole: string) => {
  return (
    globalRole === Constants.BudibaseRoles.AppUser || globalRole === Constants.BudibaseRoles.Creator
  )
}
    ...
    if (!disableRole && hasRoleChanges) {
      ...
      if (currentWorkspaceRole !== desiredWorkspaceRole) {
        if (desiredWorkspaceRole) {
          await users.addUserToWorkspace(user._id, desiredWorkspaceRole, updated._rev)
        } else {
