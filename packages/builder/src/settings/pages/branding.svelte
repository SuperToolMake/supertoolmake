...
const imageExtensions = [".png", ".tiff", ".gif", ".raw", ".jpg", ".jpeg", ".svg", ".bmp", ".jfif"]
...
$: logo = config.logoUrl ? { url: config.logoUrl, type: "image", name: "Logo" } : null

$: favicon = config.faviconUrl ? { url: config.faviconUrl, type: "image", name: "Favicon" } : null
...
{#if sdk.users.isAdmin($auth.user) && mounted}
  <Layout noPadding gap="S">
    <div class="branding fields">
      ...
      <div>
        <Toggle
          text={"Remove SuperToolMake brand from emails"}
          on:change={e => {
            let clone = { ...config }
