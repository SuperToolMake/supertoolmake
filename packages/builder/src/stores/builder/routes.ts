...
export class RoutesStore extends DerivedBudiStore<RoutesState, DerivedRoutesStore> {
  constructor() {
    const makeDerivedStore = () => {
      return derived([screenStore, workspaceAppStore], ([$screenStore, $wsa]) => {
        const workspaceApp = $wsa.selectedWorkspaceApp
        return $screenStore.screens
          .filter((s: Screen) => workspaceApp && workspaceApp._id === s.workspaceAppId)
