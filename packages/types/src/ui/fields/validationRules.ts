...
export const defaultErrorForConstraint = (
  ...
  switch (constraint) {
    ...
    case "maxLength":
      return hasValidationValue(value) ? `Must be at most ${value} characters` : "Too long"
    case "minValue":
      if (type === FieldType.DATETIME) {
        return hasValidationValue(value) ? `Must be no earlier than ${value}` : "Value too low"
      }
      return hasValidationValue(value) ? `Must be at least ${value}` : "Value too low"
    case "maxValue":
      if (type === FieldType.DATETIME) {
        return hasValidationValue(value) ? `Must be no later than ${value}` : "Value too high"
      }
      return hasValidationValue(value) ? `Must be at most ${value}` : "Value too high"
    case "equal":
      return hasValidationValue(value) ? `Must equal ${value}` : "Invalid value"
    case "notEqual":
      return hasValidationValue(value) ? `Must not equal ${value}` : "Invalid value"
    case "regex":
    case "notRegex":
      return "Invalid format"
    case "contains":
      return hasValidationValue(value) ? `Must contain "${value}"` : "Missing required content"
    case "notContains":
      return hasValidationValue(value) ? `Must not contain "${value}"` : "Invalid content"
    case "maxFileSize":
      return hasValidationValue(value) ? `Files must be smaller than ${value} MB` : "File too large"
    case "maxUploadSize":
