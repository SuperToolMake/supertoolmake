...
describe("workspaceApps utils", () => {
  ...
  it("should create additional workspace apps", async () => {
    for (const url of ["/app", "/app2"]) {
      workspaceApps.push(
        (await config.api.workspaceApp.create(structures.workspaceApps.createRequest({ url })))
          .workspaceApp
      )
    }
  })

  describe.each(["", "/"])("getMatchedWorkspaceApp (url closing char: %s)", (closingChar) => {
    it("should be able to get the base workspaceApp", async () => {
      await config.doInContext(config.getDevWorkspaceId(), async () => {
        const result = await getMatchedWorkspaceApp(`/${config.getDevWorkspaceId()}${closingChar}`)
