...
import { type Datasource, FieldType, RelationshipType, type Table } from "@supertoolmake/types"
...
if (descriptions.length) {
  describe.each(descriptions)("buildSqlFieldList ($dbName)", ({ config, dsProvider }) => {
    ...
    const buildSqlFieldListInApp: typeof buildSqlFieldList = async (table, allTables, opts) => {
    ...
    describe("table", () => {
      it("extracts fields from table schema", async () => {
        ...
        expect(result).toEqual(["table.name", "table.description", "table.amount", "table.id"])
      })

      it("excludes hidden fields", async () => {
        const table = await new TableConfig("table").withHiddenField("description").create()
        const result = await buildSqlFieldListInApp(table, {})
      ...
      it("includes relationships fields when flagged", async () => {
        ...
        const table = await new TableConfig("table").withRelation("link", otherTable._id!).create()
