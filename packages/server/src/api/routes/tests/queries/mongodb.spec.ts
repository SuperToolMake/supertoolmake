...
if (descriptions.length) {
  describe.each(descriptions)("/queries ($dbName)", ({ config, dsProvider }) => {
    ...
    async function withClient<T>(callback: (client: MongoClient) => Promise<T>): Promise<T> {
    ...
    async function withCollection<T>(callback: (collection: Collection) => Promise<T>): Promise<T> {
    ...
    describe("preview", () => {
      it("should generate a nested schema with an empty array", async () => {
        ...
        await withCollection(async (collection) => await collection.insertOne({ name, nested: [] }))
      ...
      it("should update schema when structure changes from object to array", async () => {
        ...
        await withCollection(async (collection) => {
          await collection.updateOne({ name }, { $set: { field: ["value1", "value2"] } })
        })
      ...
      it("can handle all bson field types with transformers", async () => {
        ...
        await withCollection(async (collection) => {
          await collection.insertOne({
            ...
            binaryField: new BSON.Binary(new TextEncoder().encode("bufferValue")),
            ...
            codeField: new BSON.Code("function() { return 'Hello, World!'; }"),
            codeWithScopeField: new BSON.Code("function(x) { return x * 2; }", { x: 10 }),
          })
        })
        ...
        it("does not allow quoted multi-object updateMany parameters to widen the filter", async () => {
          const query = await createQuery({
            fields: {
