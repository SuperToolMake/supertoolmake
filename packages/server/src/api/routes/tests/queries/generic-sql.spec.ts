...
import type { Datasource, Query, QueryPreview } from "@supertoolmake/types"
...
if (descriptions.length) {
  describe.each(descriptions)("queries ($dbName)", ({
    config,
    dsProvider,
    isMSSQL,
    isPostgres,
    ...
    async function createQuery(query: Partial<Query>, expectations?: Expectations): Promise<Query> {
      ...
      return await config.api.query.save({ ...defaultQuery, ...query }, expectations)
    }
    ...
    describe("query verbs", () => {
      describe("create", () => {
        ...
        it.each([
          "2021-02-05T12:01:00.000Z",
          "2021-02-05",
        ])("should coerce %s into a date", async (datetimeStr) => {
          ...
          const rows = await client(tableName).where({ birthday: datetimeStr }).select()
          expect(rows).toHaveLength(1)
        ...
        it.each([
          "2021,02,05",
          "202205-1500",
        ])("should not coerce %s as a date", async (notDateStr) => {
          const query = await createQuery({
            fields: {
              sql: client(tableName)
          ...
          const rows = await client(tableName).where({ name: notDateStr }).select()
          expect(rows).toHaveLength(1)
        })
      ...
      describe("update", () => {
        it("should be able to update rows", async () => {
          ...
          expect(rows).toEqual([{ id: 1, name: "foo", birthday: null, number: null }])
        })
        ...
        it("should preserve multiline parameter values exactly", async () => {
          ...
          const rows = await client(tableName).where({ id: 1 }).select()
          expect(rows).toEqual([{ id: 1, name: multilineValue, birthday: null, number: null }])
        })

        it("should be able to execute an update that updates no rows", async () => {
          const query = await createQuery({
            fields: {
              sql: client(tableName).update({ name: "updated" }).where({ id: 100 }).toString(),
            },
            queryVerb: "update",
          })
