...
if (mainDescriptions.length) {
  describe.each(mainDescriptions)("/postgres integrations ($dbName)", ({ config, dsProvider }) => {
    ...
    describe("json column sorting behaviour", () => {
      ...
      beforeAll(async () => {
        tableName = `jsonsort_${generator.guid().replaceAll("-", "").substring(0, 12)}`
    ...
    describe("json primary key fallbacks", () => {
      ...
      beforeAll(async () => {
        tableName = `jsonprimary_${generator.guid().replaceAll("-", "").substring(0, 12)}`
  ...
  if (descriptions.length) {
    describe.each(descriptions)("Integration compatibility with postgres search_path", ({
      config,
      dsProvider,
    }) => {
      let datasource: Datasource
      ...
      it("discovers tables from any schema in search path", async () => {
        ...
        expect(response.tableNames).toEqual(expect.arrayContaining(["table1", "table2"]))
      })

      it("does not mix columns from different tables", async () => {
        const repeated_table_name = "table_same_name"

        await client.schema.createTable(`${schema1}.${repeated_table_name}`, (table) => {
        ...
        expect(response.datasource.entities?.[repeated_table_name].schema).toBeDefined()
        const schema = response.datasource.entities?.[repeated_table_name].schema
        expect(Object.keys(schema || {}).sort()).toEqual(["id", "val1"])
      })

      it("handles quoted schema names in the search path", async () => {
        const specialSchema = `bb_${generator
          .guid()
          .replaceAll("-", "")
          .substring(0, 6)}_"safe; SELECT pg_sleep(1); --`
        const tableName = "quoted_schema_table"
      ...
      it("does not execute SQL from schema names", async () => {
        ...
        try {
          datasource.config!.schema = maliciousSchema

          await config.api.datasource.info(datasource)

          const markerExists = await client.schema.hasTable(markerTable)
          expect(markerExists).toBe(false)
        } finally {
          datasource.config!.schema = `${schema1}, ${schema2}`
