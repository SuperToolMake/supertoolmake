...
if (mainDescriptions.length) {
  describe.each(mainDescriptions)("/Integration compatibility with mysql search_path ($dbName)", ({
    config,
    dsProvider,
  }) => {
    ...
    it("does not mix columns from different tables", async () => {
      ...
      await client.schema.createTable(`${database}.${repeated_table_name}`, (table) => {
  ...
  if (descriptions.length) {
    describe.each(descriptions)("POST /api/datasources/:datasourceId/schema ($dbName)", ({
      config,
      dsProvider,
    }) => {
      let datasource: Datasource
