...
function getBindingWithinConstCharRegex(sourceName: SourceName, binding: string) {
  if (sourceName === SourceName.MYSQL) {
...
export async function interpolateSQL(
  ...
  for (const binding of bindings) {
    ...
    if (charConstBindingMatch) {
      ...
      sql = sql.replace(
        ...
        integration.getStringConcat([part1, integration.getBindingIdentifier(), part2])
      )
    }
  ...
  for (let i = 0; i < variables.length; i++) {
    if (arrays.includes(variables[i])) {
      updated = updated.concat(fields.bindings[i].split(",").map((val: string) => val.trim()))
    } else {
