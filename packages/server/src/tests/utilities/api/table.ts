...
export class TableAPI extends TestAPI {
  ...
  migrate = async (
    ...
    return await this._post<MigrateTableResponse>(`/api/tables/${tableId}/migrate`, {
      body: data,
      expectations,
    })
  }
  ...
  destroy = async (tableId: string, revId: string, expectations?: Expectations): Promise<void> => {
  ...
  validateNewTableImport = async (
    ...
    return await this._post<ValidateTableImportResponse>(`/api/tables/validateNewTableImport`, {
      body,
      expectations,
    })
  }
  ...
  duplicate = async (tableId: string, expectations?: Expectations): Promise<SaveTableResponse> => {
    return await this._post<SaveTableResponse>(`/api/tables/${tableId}/duplicate`, {
      expectations,
    })
  }
