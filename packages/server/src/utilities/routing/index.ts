...
export async function getRoutingInfo(urlPath: string): Promise<ScreenRoutesViewOutput[]> {
  ...
  try {
    ...
    const allRouting = await db.query<ScreenRoutesViewOutput>(getQueryIndex(ViewName.ROUTING), {
    ...
    if (workspaceApp.isDefault) {
      const screensWithMissingWorkspaceAppRouting = await db.query<ScreenRoutesViewOutput>(
    ...
    if (err != null && err.error === "not_found" && err.reason === "missing_named_view") {
