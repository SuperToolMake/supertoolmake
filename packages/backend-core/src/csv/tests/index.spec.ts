...
describe("csv", () => {
  describe("jsonFromCsvString", () => {
    ...
    test("empty values are coerced to null", async () => {
      const csvString = '"id","optional","title"\n1,,"aaa"\n2,"value","bbb"\n3,,"ccc"'
      ...
      result.forEach((r) => expect(Object.keys(r)).toEqual(["id", "optional", "title"]))
    })
    ...
    test.each(possibleDelimeters)("Should parse with delimiter %s", async (delimiter) => {
