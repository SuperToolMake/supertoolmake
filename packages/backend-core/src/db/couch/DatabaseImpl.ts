...
export class CouchDatabase implements Database {
  ...
  static init() {
    ...
    CouchDatabase.nano = buildNano(couchInfo)
  }
  ...
  private nano() {
    return this.instanceNano || CouchDatabase.nano
  }
  ...
  async sqlDiskCleanup(): Promise<void> {
    ...
    await this._query<void>(url, "POST")
  }
  ...
  async sqlPurgeDocument(docIds: string[] | string): Promise<void> {
    ...
    return await this._query<void>(url, "POST", { docs: docIds })
  }
