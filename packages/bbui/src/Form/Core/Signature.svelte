...
export function toFile() {
  const data = canvasContext.getImageData(0, 0, width, height).data.some((channel) => channel !== 0)
...
onMount(() => {
  ...
  const { width: wrapWidth, height: wrapHeight } = canvasWrap.getBoundingClientRect()
